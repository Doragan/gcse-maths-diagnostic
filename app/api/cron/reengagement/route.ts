...
const BATCH_LIMIT = 100         // cap per run; the daily cadence drains any backlog
...
export async function GET(req: NextRequest) {
  ...
  const batch  = cohort.slice(0, BATCH_LIMIT)
  ...
  for (const s of batch) {
    ...
    //    unsubscribe links, and the unique(student_id) index makes a concurrent
    //    double-run a no-op for this student.
    const { data: row, error: insErr } = await supabase
      .from('reengagement_sends')
      .insert({ student_id: s.student_id })
      .select('id')
      .single()
    if (insErr || !row) {
  ...
  console.log(`[reengagement] run complete — cohort=${cohort.length} sent=${sent} failed=${failed}`)
  return NextResponse.json({ ok: true, cohort: cohort.length, sent, failed })
}
