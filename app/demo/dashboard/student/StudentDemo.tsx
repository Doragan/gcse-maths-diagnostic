...
// ─── Mock Data ──────────────────────────────────────────────────────────────
//
// Dates are OFFSETS IN DAYS from the server-supplied anchor, not literals, so a
// demo opened next term is not showing last term's homework — see
// lib/demoDates.ts. The offsets deliberately match the teacher demo's, since
// Ben Okonkwo appears in both and the two views should agree.
const STUDENT = { name: 'Ben', fullName: 'Ben Okonkwo', yearGroup: 'Year 10', teacher: 'Mr Thompson' }

const buildAssessments = (a: string): Assessment[] => [
  {
    id: 'paper3-nov24', type: 'exam',
    title: 'Paper 3 Calculator (Nov 2024)', date: demoDate(a, -18),
    score: 24, total: 35,
    topics: {
  ...
  },
  {
    ...
    title: 'Fractions and Decimals Homework', date: demoDate(a, -24),
    score: 7, total: 10,
  ...
  },
  {
    ...
    title: 'Algebra End of Topic Test', date: demoDate(a, -31),
    score: 9, total: 20,
  ...
  },
  {
    ...
    title: 'Paper 1 Non-Calculator (Nov 2024)', date: demoDate(a, -61),
    score: 31, total: 80,
    topics: {
  ...
  },
  {
    ...
    title: 'Angles in Polygons Homework', date: demoDate(a, -95),
    score: 4, total: 8,
  ...
  },
  {
    ...
    title: 'Paper 2 Calculator (Jun 2024)', date: demoDate(a, -126),
    score: 25, total: 80,
    topics: {
...
const buildHomework = (a: string): HomeworkItem[] => [
  { title: 'Sequences Homework', topic: 'Algebra', topicColour: TOPIC_COLOURS.algebra.fg, dueDate: demoDate(a, 9), status: 'due', url: '/practice/question/321580e4-cb6d-4f9e-a3c6-d558ae8ba553' },
  { title: 'Ratio and Proportion Homework', topic: 'Ratio and Proportion', topicColour: TOPIC_COLOURS.ratio.fg, dueDate: demoDate(a, 16), status: 'due', url: '/practice/question/a653ffa7-038e-434e-90a5-d692e9fa8675' },
  { title: 'Fractions and Decimals Homework', topic: 'Number', topicColour: TOPIC_COLOURS.number.fg, dueDate: demoDate(a, -24), status: 'completed', score: 7, total: 10, url: '/practice/question/463e9833-bc0a-40d9-bc40-941de561a0cc' },
  { title: 'Angles in Polygons Homework', topic: 'Shape and Space', topicColour: TOPIC_COLOURS.shape.fg, dueDate: demoDate(a, -95), status: 'completed', score: 4, total: 8, url: '/practice/question/441e242a-e795-4aad-bf96-05a1a275c2cf' },
]
...
type MasteryHistory = Record<TopicId, MasteryPoint[]>

/** A point on a topic's mastery line, `d` days before the anchor. */
const pt = (a: string, d: number, pct: number): MasteryPoint =>
  ({ date: demoDateIso(a, d), label: demoMonthLabel(a, d), pct })

const buildMasteryHistory = (a: string): MasteryHistory => ({
  number:   [pt(a, -300, 35), pt(a, -240, 48), pt(a, -120, 56), pt(a, -18, 72)],
  algebra:  [pt(a, -300, 18), pt(a, -240, 25), pt(a, -120, 32), pt(a, -18, 38)],
  shape:    [pt(a, -240, 29), pt(a, -95, 42),  pt(a, -61, 45)],
  ratio:    [pt(a, -300, 30), pt(a, -240, 31), pt(a, -120, 43), pt(a, -18, 68)],
  probdata: [pt(a, -240, 40), pt(a, -120, 40), pt(a, -18, 62)],
})

// Overall mastery is the average of the latest point across all topics
function overallMastery(history: MasteryHistory): number {
...
function prevOverallMastery(history: MasteryHistory): number | null {
...
function currentMastery(history: MasteryHistory, topicId: TopicId): number {
  const hist = history[topicId]
  return hist.length > 0 ? hist[hist.length - 1].pct : 0
}

function masteryTrend(history: MasteryHistory, topicId: TopicId): 'up' | 'down' | 'same' | 'new' {
  const hist = history[topicId]
...
function masteryChange(history: MasteryHistory, topicId: TopicId): number | null {
  const hist = history[topicId]
...
function MasteryChart({ history, width = 600, height = 220 }: { history: MasteryHistory; width?: number; height?: number }) {
  // Build overall mastery at each unique date by averaging all topics that have data at that point
  const allDates: string[] = []
  const allLabels: Record<string, string> = {}
  TOPICS.forEach(tp => {
    history[tp.id].forEach(p => {
...
// ═══════════════════════════════════════════════════════════════════════════
export default function StudentDashboard({ anchor }: { anchor: string }) {
  // `anchor` is today, computed on the SERVER (see ./page.tsx) — deriving it
  // from a client-side `new Date()` would disagree with the prerendered HTML.
  const ASSESSMENTS = useMemo(() => buildAssessments(anchor), [anchor])
  const HOMEWORK = useMemo(() => buildHomework(anchor), [anchor])
  const MASTERY_HISTORY = useMemo(() => buildMasteryHistory(anchor), [anchor])
  ...
  const currentOverall = overallMastery(MASTERY_HISTORY)
  const prevOverall = prevOverallMastery(MASTERY_HISTORY)
  const improving = prevOverall !== null && currentOverall > prevOverall

  const topicScores = TOPICS.map(tp => ({ ...tp, pct: currentMastery(MASTERY_HISTORY, tp.id) }))
  ...
  return (
    <div style={{ minHeight: '100dvh', background: colors.background }}>
      <DemoDataRibbon note="Ben Okonkwo is an invented student — this is an illustrative term of results, not a real learner." />
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
            ...
            <div style={cardStyle}>
              ...
              <div style={{ borderRadius: radius.md, border: `1px solid ${colors.border}`, padding: '12px 8px 4px', background: colors.cardAlt, marginBottom: 16 }}>
                <MasteryChart history={MASTERY_HISTORY} width={680} height={220} />
              </div>

              {/* Topic cards (expandable for skill breakdown) */}
              <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                {TOPICS.map(tp => {
                  const p = currentMastery(MASTERY_HISTORY, tp.id)
                  const trend = masteryTrend(MASTERY_HISTORY, tp.id)
                  const change = masteryChange(MASTERY_HISTORY, tp.id)
            ...
            <div style={cardStyle}>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                {filteredAssessments.map(a => {
                  ...
                  return (
                    <button key={a.id} onClick={() => setSelectedAssessmentId(a.id)} style={{
                      display: 'flex', justifyContent: 'space-between', alignItems: 'center',
